...
from scenarionet.converter.vod.type import ALL_TYPE, HUMAN_TYPE, BICYCLE_TYPE, VEHICLE_TYPE
...
def convert_vod_scenario(
    ...
    if prediction:
        ...
        frames_scene_info = extract_frames_scene_info(token, vod)
    ...
    result[SD.METADATA]["map"] = vod.get("log", scene_info["log_token"])["location"]
    result[SD.METADATA]["date"] = vod.get("log", scene_info["log_token"])["date_captured"]
...
def get_vod_scenarios(dataroot, version, num_workers=2):
    vod = VOD(version=version, dataroot=dataroot)

    def _get_vod():
        return VOD(version=version, dataroot=dataroot)

    return vod.scene, [vod for _ in range(num_workers)]
